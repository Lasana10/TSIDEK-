...
type DeliveryChannel = "Email" | "WhatsApp" | "SMS" | "In-app";
...
async function processEvent(event: OutboxRow) {
  if (event.event_type === "client.update.delivery") {
    ...
    const channel = String(event.payload.channel ?? "In-app") as DeliveryChannel;
    ...
    const result = await deliverMatterUpdate({ firmId: event.firm_id, matterId: event.matter_id, channel, title, message });
...
export async function processOutboxBatch(limit = 20) {
  ...
  for (const event of events) {
    ...
    try {
      ...
      await supabase.from("domain_outbox_events").update({ status: "delivered", delivered_at: new Date().toISOString(), locked_at: null, locked_by: null, last_error: null }).eq("id", event.id);
      ...
      await supabase.from("domain_outbox_events").update({ status: terminal ? "dead_letter" : "failed", available_at: availableAt, locked_at: null, locked_by: null, last_error: message.slice(0, 1000) }).eq("id", event.id);

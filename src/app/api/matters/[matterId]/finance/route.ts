...
async function runtime(request: Request, matterId: string) {
  ...
  const matter = await supabase.from("matters").select("id,firm_id,title,client_name").eq("id", matterId).eq("firm_id", firmId).single();
  if (matter.error) throw new Error(matter.error.message);
  return { scope, firmId, supabase, matter: matter.data };
}
...
export async function POST(request:Request,context:Context){
 try{
  const {matterId}=await context.params; const rt=await runtime(request,matterId); const {scope,firmId,supabase,matter}=rt; const body=await request.json(); const action=String(body.action||"");
  ...
  if(action==="createInvoice"){
    ...
    const numberResult=await supabase.rpc("next_firm_document_number",{p_firm_id:firmId,p_prefix:"INV",p_table:"invoice"}); if(numberResult.error)throw new Error(numberResult.error.message);
    const invoice=await supabase.from("invoices").insert({firm_id:firmId,matter_id:matterId,invoice_number:numberResult.data,amount_xaf:amount,status:"Sent",due_date:body.dueDate||null,description:String(body.description||"Legal fees"),issued_at:new Date().toISOString(),created_by:scope.actorLawyerId}).select("*").single(); if(invoice.error)throw new Error(invoice.error.message);
    const ledger=await supabase.from("finance_ledger_entries").upsert({firm_id:firmId,matter_id:matterId,entry_type:"INVOICE",source_table:"invoices",source_id:invoice.data.id,amount_xaf:amount,direction:"DEBIT",account_bucket:"RECEIVABLE",status:"POSTED",description:`Invoice ${invoice.data.invoice_number}`,created_by:scope.actorLawyerId},{onConflict:"firm_id,source_table,source_id,entry_type"}); if(ledger.error)throw new Error(ledger.error.message);
    ...
    const tenantCredentials=await resolveFirmProviderCredentials(firmId,"pawapay");

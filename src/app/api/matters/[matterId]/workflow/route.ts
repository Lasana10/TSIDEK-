...
type AtomicResult = { instance?: unknown; event?: unknown; target_stage?: string } | null;
...
async function loadRuntime(request: Request, matterId: string) {
  ...
  if (!instance.data) {
    ...
    const created = await supabase.from("matter_workflow_instances").insert({ firm_id: scope.firmId, matter_id: matterId, workflow_id: workflow.data.id, current_stage_key: "opened" }).select("*").single();
    if (created.error) throw new Error(created.error.message);
    instance = created;
    const event = await supabase.from("matter_workflow_events").insert({ firm_id: scope.firmId, matter_id: matterId, instance_id: created.data.id, from_stage_key: null, to_stage_key: "opened", action: "initialized", actor_lawyer_id: scope.actorLawyerId, actor_role: scope.actorRole ?? null });
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (action === "reviewApproval") {
      ...
      const atomic = await runtime.supabase.rpc("transition_matter_workflow_atomic", { p_instance_id: runtime.instance.id, p_transition_id: transition.id, p_expected_stage: runtime.instance.current_stage_key, p_reason: pending.reason ?? "Approved workflow transition", p_guard_snapshot: guard, p_actor_lawyer_id: runtime.scope.actorLawyerId, p_actor_role: runtime.scope.actorRole ?? null });
      if (atomic.error) throw new Error(atomic.error.message);
      const result = atomic.data as AtomicResult;
    ...
    if (transition.requires_approval && approvalRoles.length && !approvalRoles.includes(role)) {
      const existingPending = runtime.requests.find((item) => item.transition_id === transition.id && item.status === "pending");
      const payload = { firm_id: runtime.scope.firmId, matter_id: matterId, instance_id: runtime.instance.id, transition_id: transition.id, from_stage_key: runtime.instance.current_stage_key, to_stage_key: transition.to_stage_key, requester_lawyer_id: runtime.scope.actorLawyerId, requester_role: runtime.scope.actorRole ?? null, reason: reason || null, guard_snapshot: guard, status: "pending", updated_at: new Date().toISOString() };
      const requestResult = existingPending
        ? await runtime.supabase.from("workflow_transition_requests").update(payload).eq("id", existingPending.id).select("*").single()
        : await runtime.supabase.from("workflow_transition_requests").insert(payload).select("*").single();
      if (requestResult.error) throw new Error(requestResult.error.message);
      return NextResponse.json({ success: true, approvalRequired: true, approvalRequest: requestResult.data }, { status: 202 });
    }

    const atomic = await runtime.supabase.rpc("transition_matter_workflow_atomic", { p_instance_id: runtime.instance.id, p_transition_id: transition.id, p_expected_stage: runtime.instance.current_stage_key, p_reason: reason, p_guard_snapshot: guard, p_actor_lawyer_id: runtime.scope.actorLawyerId, p_actor_role: runtime.scope.actorRole ?? null });
    if (atomic.error) throw new Error(atomic.error.message);
    const result = atomic.data as AtomicResult;
    const targetStage = String(result?.target_stage ?? transition.to_stage_key);
    const actions = await executeWorkflowStageActions({ supabase: runtime.supabase, firmId: runtime.scope.firmId!, matterId, instanceId: runtime.instance.id, workflowId: runtime.instance.workflow_id, stageKey: targetStage, actorLawyerId: runtime.scope.actorLawyerId, leadLawyerId: runtime.matter.lead_lawyer_id });

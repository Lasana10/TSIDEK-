...
function filterRoomDocumentsForScope(scope: Awaited<ReturnType<typeof resolveRequestScope>>, room: Awaited<ReturnType<typeof getMatterRoomById>>) {
  ...
  if (isPartner || isLeadLawyer) return room;
  return { ...room, documents: room.documents.filter((document) => document.accessLevel !== "Lead+Partner") };
}
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : "Unable to load file vault." }, { status: statusForApiError(error) });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || !file.size) return NextResponse.json({ success: false, error: "A file is required." }, { status: 400 });
    ...
    if (target === "matter-document") {
      ...
      await createMatterDocument({
        ...
        documentStatus: String(formData.get("documentStatus") ?? "Draft") as "Draft" | "Final" | "Filed" | "Archived",
        reviewStatus: String(formData.get("reviewStatus") ?? "Working") as "Working" | "Internal review" | "Approved" | "Needs revision",
        accessLevel: String(formData.get("accessLevel") ?? "Matter team") as "Matter team" | "Lead+Partner",
        sharingPolicy: String(formData.get("sharingPolicy") ?? "Internal only") as "Internal only" | "Client-share ready" | "Blocked",
        versionLabel: String(formData.get("versionLabel") ?? "v1").trim() || "v1",
        storagePath: stored.relativePath,
        aiSummary: String(formData.get("aiSummary") ?? "").trim() || `${file.name} was uploaded into the governed matter vault.`,
    ...
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : "Unable to process file upload." }, { status: statusForApiError(error) });
  }
}

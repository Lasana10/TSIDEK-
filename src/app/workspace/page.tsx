...
import { AlertTriangle, Archive, BriefcaseBusiness, Building2, CheckCircle2, Clock3, FileCheck2, FileText, Landmark, MessageSquareText, Receipt, ShieldCheck, Sparkles, Users, LibraryBig } from "lucide-react";
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#eef3f2_100%)] p-4 text-slate-800 md:p-6 xl:p-8">
      <div className="mx-auto max-w-[1700px] space-y-6">
        <section className="rounded-[2rem] border border-white/70 bg-[#082b22] p-6 text-white shadow-[0_24px_60px_rgba(0,43,34,0.18)] md:p-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <Link href="/law-bank" className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm font-semibold text-white">Law Bank</Link>
              <Link href="/people" className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm font-semibold text-white">People</Link>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <Link href="/law-bank" className="rounded-[1.4rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300"><LibraryBig className="h-5 w-5 text-[#0f5b49]"/><h2 className="mt-4 text-lg font-semibold text-slate-950">Law Bank</h2><p className="mt-2 text-sm leading-6 text-slate-500">Source-governed legislation, OHADA/CEMAC/OAPI material, jurisprudence and internal precedent.</p></Link>
          <Link href="/people" className="rounded-[1.4rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300"><Users className="h-5 w-5 text-[#0f5b49]"/><h2 className="mt-4 text-lg font-semibold text-slate-950">People & performance</h2><p className="mt-2 text-sm leading-6 text-slate-500">Supervision and professional development based on real work, timeliness and quality—not AI usage.</p></Link>
        </section>

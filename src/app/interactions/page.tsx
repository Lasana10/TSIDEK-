...
import {
  ArrowLeft, CheckCircle2, MessageSquareText, Mic, PauseCircle, Phone, PlayCircle,
...
export default function InteractionsPage() {
  ...
  async function uploadCapturedRecording(interactionId:string) {
    ...
    return payload.transcription as { status?:string; configured?:boolean; error?:string|null } | undefined;
  }
  ...
  async function saveInteraction() {
    ...
    try {
      ...
      const p=await r.json(); if(!p.success) throw new Error(p.error);
      let transcription: {status?:string;configured?:boolean;error?:string|null}|undefined;
      ...
      else if (recordingBlob && transcription?.configured === false) setMessage("Interaction and audio saved. Transcription is not configured for this runtime.");
      else if (recordingBlob && transcription?.status === "failed") setMessage(`Interaction and audio saved, but transcription failed: ${transcription.error || "provider error"}`);
      else setMessage(recordingBlob ? "Interaction and authorized audio saved." : "Interaction saved.");
    } catch(e) { setMessage(e instanceof Error?e.message:"Unable to save interaction."); }
    finally { setSaving(false); if(captureState==="uploading") setCaptureState(recordingBlob?"ready":"idle"); }
  }
